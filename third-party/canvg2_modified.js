...
(function (global, factory) {
	...
	function build(opts) {
	    ...
	    this.setContext = function (ctx, fromMeasure) {
	      if (!fromMeasure) { // causes stack overflow when measuring text with gradients
	        ...
	        if (this.style('stroke-linecap').hasValue()) {
	        	ctx.lineCap2 = this.style('stroke-linecap').value;
	        }
	        if (this.style('stroke-linejoin').hasValue()) {
	        	ctx.lineJoin2 = this.style('stroke-linejoin').value;
	        }
	        if (this.style('stroke-miterlimit').hasValue()) {
	        	ctx.miterLimit2 = this.style('stroke-miterlimit').value;
	        }
	      ...
	      // font
	      if (typeof ctx.font2 != 'undefined' && ctx.font2 !== '10px sans-serif') {
	      	console.log('setting font 2');
	  ...
	  svg.Element.svg = function (node) {
	    ...
	    this.setContext = function (ctx) {
	      svg.SetDefaults(ctx);
	      if (ctx.canvas.style && typeof ctx.font2 != 'undefined'
	      	&& ctx.font2 !== "10px sans-serif"       // Modified: set only when it is not the default value
	      	&& typeof windowEnv.getComputedStyle != 'undefined') {
	      	console.log("setting font 1, for " + ctx.canvas.name + "prev val is " + ctx.font2);
...
draw();
if(window.onSVGDraw){
onSVGDraw("function(ctx){ " + code + "};", ctx.canvas);
}
	    }
	    {
...
function customCanvas(ctx, tempCtx) {
    ...
    if (tempCtx) {
        ...
        'canvas.height = ' + ctx.canvas.height + '; var ' + ctx.name + ' = canvas.getContext("2d"); ', ctx);
    }
    ...
    function valueToString(v) {
        if (typeof v == 'string') {
            ...
            if(!images[v.src]){
                images[v.src] = v;
                v.name = 'img' + index++;
                appendCode('var ' + v.name + '= new Image(); ' + v.name + '.src = ' + valueToString(v.src) + '; ', ctx);
            }
            return v.name;
        }
        return v;
    ...
    function createFunction(name) {
        return function () {
            ...
            if (name == 'createLinearGradient' || name == 'createRadialGradient') {
                appendCode('var g = ' + ctxName + '.' + name + '(' + param + '); ', this);
                ...
                gradient.addColorStop = function (offset, color) {
                    appendCode('g.addColorStop(' + valueToString(offset) + ',' + valueToString(color) + '); ', ctx);
                    return oldFunction.apply(this, arguments);
                }
                return gradient;
            } else if (name == 'createPattern') {
                var result = this[name].apply(this, arguments);
                appendCode('var p = ' + ctxName + '.' + name + '(' + param + '); ', this);
                return result;
                ...
                appendCode('' + ctxName + '.' + name + '([' + param + ']); ', this);
    ...
    function createGetSet(name) {
        return {
            ...
            set: function (v) {
                var old = this[name];
                this[name] = v;
                if (this[name] === old) { return; }
                var ctxName = this.name;
                appendCode(ctxName + '.' + name + '=' + valueToString(v) + '; ', this);
            }
        }
    }
